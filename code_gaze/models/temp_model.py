...
class Model(nn.Module):
    def __init__(self):
        ...
        self.conv4 = nn.Conv2d(64, 1, kernel_size=3, stride=1,
                               padding=1)  # 若卷积核大小为3x3, 那么就应该设定padding=1, 即填充1层边缘像素; 若卷积核大小为7x7, 那么就应该设定padding=3
        # num_capsules=8 数字胶囊的个数; num_route_nodes: 一个立方体的体积; in_channels=4 初始胶囊立方体个数; out_channels=4 数字胶囊的长度
        self.conctod = nn.Conv2d(64, d, kernel_size=1).cuda()
        self.condtoc = nn.Conv2d(d2, 64, kernel_size=1).cuda()
    ...
    def forward(self, x, y):
        ...
        d2 = int(d * (d + 1) / 2)
        x = self.conctod(x)
        Y1 = x.permute(0, 2, 3, 1).unsqueeze(3)  # (64,9,15,1,64)
        Y2 = x.permute(0, 2, 3, 1).unsqueeze(4)  # (64,9,15,64,1)
        Y = Y1.mul(Y2)  # torch.Size([64, 9, 15, 64, 64])
        index = torch.triu(torch.ones((batch_size, 9, 15, d, d)) == 1)
        z = Y[index]
        z = z.reshape((batch_size, 9, 15, d2))
        z = z.permute(0, 3, 1, 2)
        A = self.condtoc(z)  # torch.Size([64, 64, 9, 15])
        G = self.avg(A)  # torch.Size([64, 64, 1, 1])
        D = torch.mul(A, G) + A  # torch.Size([64, 64, 9, 15])
        D = F.softmax(D, dim=1)
        x = torch.mul(x, D) + x  # torch.Size([64, 64, 9, 15])

        # print(z.shape)
        # z = torch.ones((batch_size, 9,15, d2)).cuda()
        # k = 0
        # for row in range(d):
